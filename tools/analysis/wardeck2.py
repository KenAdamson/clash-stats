...
for cards,n,opp_wins in rows:
    ...
    matched=[c for c in cards if c in POOL]
    missing=[c for c in cards if c not in POOL]
    lvl=sum(POOL[c] for c in matched)
    decks.append((len(matched),lvl,opp_wins/n,n,cards,missing))
print(f"decks analyzed: {len(decks)}")
def top(label, wcpred, k=8):
    print(f"\n===== {label} =====")
    sub=[d for d in decks if wcpred(d[4]) and d[0]>=6]
    sub.sort(key=lambda x:(-x[0],-x[2],-x[3]))   # matched, then WR, then popularity
    for m,lvl,wr,n,cards,missing in sub[:k]:
        star=lambda c: c+("" if c in POOL else "*")
        print(f"  match {m}/8 Σlvl={lvl} corpusWR={wr*100:.0f}% n={n}  sub:{missing if missing else '—'}")
        print(f"     {', '.join(star(c) for c in cards)}")
top("LAVA HOUND beatdown", lambda c:"Lava Hound" in c)
top("ELECTRO GIANT beatdown", lambda c:"Electro Giant" in c)
top("GOLEM beatdown", lambda c:"Golem" in c)
top("X-BOW siege/cycle", lambda c:"X-Bow" in c)
top("MORTAR cycle", lambda c:"Mortar" in c)
