...
export WP_SHARD_DIR=/app/data/wp_shards_oriented
export WP_FROM_SCRATCH=1
export WP_DECK_FEATURES=0
export WP_DECK_INTERACTION=0
export WP_DECK_ANTISYM=0
...
if [ ! -f "${WP_RESUME}" ]; then
  echo "[v9x $(date -u +%FT%TZ)] FATAL: warm-start checkpoint ${WP_RESUME} missing" >> "$log"
  exit 1
fi
if [ ! -f /app/data/wp_shards/deck_ids.npy ]; then
  echo "[v9x $(date -u +%FT%TZ)] FATAL: shards carry no deck prior — run" \
       "tools/ml/backfill_shard_decks.py first" >> "$log"
  exit 1
fi

{
  echo "[v9x $(date -u +%FT%TZ)] starting: deck prior, warm start from $(basename "${WP_RESUME}")"
...
rc=$?
if [ $rc -eq 99 ]; then
  echo "[v9x $(date -u +%FT%TZ)] could not take ${XPU_TRAIN_LOCK} — another XPU training job is live" >> "$log"
elif [ $rc -ne 0 ]; then
  echo "[v9x $(date -u +%FT%TZ)] TRAINING FAILED rc=${rc} — see traceback above" >> "$log"
fi
echo "[v9x $(date -u +%FT%TZ)] exited rc=${rc}" >> "$log"
exit $rc
