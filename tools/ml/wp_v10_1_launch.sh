...
if [ ! -f "${WP_RESUME}" ]; then
  echo "[v10.1 $(date -u +%FT%TZ)] FATAL: warm-start checkpoint ${WP_RESUME} missing" >> "$log"
  exit 1
fi
if [ ! -f /app/data/wp_shards/deck_ids.npy ]; then
  echo "[v10.1 $(date -u +%FT%TZ)] FATAL: shards carry no deck prior — run" \
       "tools/ml/backfill_shard_decks.py first" >> "$log"
  exit 1
fi

{
  echo "[v10.1 $(date -u +%FT%TZ)] starting: deck prior, warm start from $(basename "${WP_RESUME}")"
...
rc=$?
if [ $rc -eq 99 ]; then
  echo "[v10.1 $(date -u +%FT%TZ)] could not take ${XPU_TRAIN_LOCK} — another XPU training job is live" >> "$log"
elif [ $rc -ne 0 ]; then
  echo "[v10.1 $(date -u +%FT%TZ)] TRAINING FAILED rc=${rc} — see traceback above" >> "$log"
fi
echo "[v10.1 $(date -u +%FT%TZ)] exited rc=${rc}" >> "$log"
exit $rc
